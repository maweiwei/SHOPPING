<html>
	...
	<body>
	    ...
	    <hr class="c"/>
	<center>
	    <font size="7" color="#f3168a">购买历史</font></center>
		<hr class="d"/>
		
		<div id="liucheng">
			<div style="float:right"><a href="{{ url("/Home/indent") }}" style="background:#f3168a;border-radius:5px;color:#FFF;">查看订单</a></div>
		</div>
		@foreach($history as $tmp)
		<div class="c" id="body">
			<div id="body-top">
				<div id="b_t1">唯品会</div>
				<div id="b_t2">订单号：{{ $tmp->oid }}&nbsp&nbsp订单日期：{{ $tmp->odate }}</div>
				<div id="b_t6">操作</div>
				<div id="b_t5">小计</div>
				<div id="b_t4">数量</div>
				<div id="b_t3">单价</div>
			...
			<div id="body-middle"> 
				...
				@if($tmp->send == 1)
					     <a href="{{ url("/Home/gopl/{$tmp->sid}") }}"><input type="button" value="去评论" style="background:#f3168a;color:#FFF;"/></a>
				@else
					     <a href=""><input type="button" value="查看评论" style="background:#f3168a;color:#FFF;border-radius:5px;"/></a>
				@endif
					     </div>
					 </div>
				</div>
